...
export const formatResponse = {
	...
	contextTruncationNotice: () =>
		`[注意] 为了维持最佳上下文窗口长度，部分之前与用户的对话历史已被移除。为了保持连续性，我们保留了初始用户任务和最近的交流内容，而中间的对话历史已被移除。请在继续协助用户时记住这一点。`,

	condense: () =>
		`用户已接受你生成的对话摘要。此摘要涵盖了已被截断的历史对话中的重要细节。\n<explicit_instructions type="condense_response">至关重要的是，你的回应应该仅限于询问用户接下来应该做什么。你不应该主动采取行动或对继续工作做出任何假设。例如，你不应该建议文件更改或尝试读取任何文件。\n当询问用户接下来应该做什么时，你可以引用刚刚生成的摘要中的信息。但是，在此回应中，你不应该引用摘要之外的信息。请保持回应简洁。</explicit_instructions>`,
	...
	clineIgnoreError: (path: string) =>
		`访问 ${path} 被 .clineignore 文件设置阻止。你必须尝试在不使用此文件的情况下继续任务，或请用户更新 .clineignore 文件。`,

	noToolsUsed: () =>
		`[错误] 你在上一个回应中没有使用工具！请使用工具重试。
...
（这是一条自动消息，请不要以对话方式回应。）`,
	...
	invalidMcpToolArgumentError: (serverName: string, toolName: string) =>
		`对 ${serverName} 的 ${toolName} 使用了无效的 JSON 参数。请使用格式正确的 JSON 参数重试。`,
	...
	formatFilesList: (
		...
		const sorted = files
			...
			// 排序，使文件列在其各自目录下，清楚地显示哪些文件是哪些目录的子文件。由于我们自上而下构建文件列表，即使文件列表被截断，它也会显示cline可以进一步探索的目录。
			.sort((a, b) => {
				const aParts = a.split("/") // 仅在先使用toPosix时有效
				const bParts = b.split("/")
				for (let i = 0; i < Math.min(aParts.length, bParts.length); i++) {
					if (aParts[i] !== bParts[i]) {
						// 如果在此级别上一个是目录而另一个不是，则先排目录
	...
	taskResumption: (
		...
		const taskResumptionMessage = `[任务恢复] ${
			mode === "plan"
				? `此任务在${agoText}前被中断。对话可能不完整。请注意，项目状态可能已经改变。当前工作目录现在是'${cwd.toPosix()}'。\n\n注意：如果你之前尝试使用了某个工具但用户没有提供结果，你应该假设该工具使用不成功。但是你现在处于规划模式，所以不要继续任务，而是必须回应用户的消息。`
				: `此任务在${agoText}前被中断。它可能已完成也可能未完成，请重新评估任务上下文。请注意，项目状态可能已经改变。当前工作目录现在是'${cwd.toPosix()}'。如果任务尚未完成，请重试中断前的最后一步，然后继续完成任务。\n\n注意：如果你之前尝试使用了某个工具但用户没有提供结果，你应该假设该工具使用不成功并评估是否应该重试。如果最后一个工具是browser_action，浏览器已被关闭，如果需要，你必须启动一个新的浏览器。`
		}${
			wasRecent
...
（记住：如果用户似乎希望你使用只在执行模式下可用的工具，你应该请用户"切换到执行模式"（使用这些词）- 他们将需要使用下方的规划/执行切换按钮手动完成此操作。你无法自行切换到执行模式，必须等待用户在对计划满意后自行切换。你也不能提供切换到执行模式的选项，因为这是用户需要自行手动完成的事情。）`
	},
	...
	fileEditWithUserChanges: (
		...
		`用户对你的内容进行了以下更新：\n\n${userEdits}\n\n` +
		(autoFormattingEdits
			? `用户的编辑器还对你的内容应用了以下自动格式化：\n\n${autoFormattingEdits}\n\n（注意：请密切关注诸如单引号被转换为双引号、分号被移除或添加、长行被分成多行、调整缩进样式、添加/删除尾随逗号等变化。这将帮助你确保对此文件的未来SEARCH/REPLACE操作准确无误。）\n\n`
			: "") +
		...
		`3. 如果用户的编辑已解决部分任务或改变了需求，请相应调整你的方法。` +
		`4. 重要：对于此文件的任何未来更改，请使用上面显示的final_file_content作为参考。此内容反映了文件的当前状态，包括用户编辑和任何自动格式化（例如，如果你使用了单引号但格式化器将其转换为双引号）。始终基于此最终版本进行SEARCH/REPLACE操作以确保准确性。\n` +
		`${newProblemsMessage}`,
	...
	diffError: (relPath: string, originalContent: string | undefined) =>
		`这可能是因为SEARCH块内容与文件中的内容不完全匹配，或者如果你使用了多个SEARCH/REPLACE块，它们可能不是按照它们在文件中出现的顺序排列的。（还请确保在使用replace_in_file工具时，不要在标记中添加额外字符（例如，------- SEARCH>是无效的）。不要忘记使用结束标记+++++++ REPLACE。不要以任何方式修改标记格式。格式错误的XML将导致工具完全失败并破坏整个编辑过程。）\n\n` +
		`文件已恢复到其原始状态：\n\n` +
		`<file_content path="${relPath.toPosix()}">\n${originalContent}\n</file_content>\n\n` +
		`现在你已经有了文件的最新状态，请尝试使用更少、更精确的SEARCH块再次操作。特别是对于大文件，限制自己一次使用<5个SEARCH/REPLACE块可能是明智的，然后等待用户对操作结果做出响应，再跟进另一个replace_in_file调用进行额外编辑。\n（如果你连续3次遇到此错误，你可以使用write_to_file工具作为备选方案。）`,
